...
export default async function handler(req, res) {
  ...
  if (origin && !allowedOrigins.includes(origin)) return res.status(403).json({ error: 'Forbidden', apiVersion: 'v7.8.5' });

  let { symbol } = req.query;
  if (!symbol) return res.status(400).json({ error: 'Missing symbol', apiVersion: 'v7.8.5' });
  ...
  try {
    if (/^\d{4}$/.test(queryTerm) || /[\u4e00-\u9fa5]/.test(queryTerm)) {
      ...
      if (!response || !response.ok) return res.status(400).json({ error: `找不到代號 ${finalSymbol} 的市場資料`, apiVersion: 'v7.8.5' });
      
      const data = await response.json();
      const meta = data.chart?.result?.[0]?.meta;
      if (!meta) return res.status(400).json({ error: '查無資料', apiVersion: 'v7.8.5' });

      const currentPrice = meta.preMarketPrice || meta.postMarketPrice || meta.regularMarketPrice || meta.chartPreviousClose;
      return res.status(200).json({ symbol: meta.symbol || finalSymbol, name: resolvedName, currentPrice: Number(currentPrice), prevClose: Number(meta.chartPreviousClose || currentPrice), apiVersion: 'v7.8.5 (TW)' });

    } else {
      const robinhoodData = await fetchRobinhoodPrice(finalSymbol);
      if (robinhoodData) return res.status(200).json({ ...robinhoodData, apiVersion: 'v7.8.5 (Robinhood 24h)' });

      const webullData = await fetchWebull24hPrice(finalSymbol);
      if (webullData) return res.status(200).json({ ...webullData, apiVersion: 'v7.8.5 (Webull 24h)' });
      ...
      if (!response || !response.ok) return res.status(400).json({ error: `查無資料`, apiVersion: 'v7.8.5' });

      const data = await response.json();
      const meta = data.chart?.result?.[0]?.meta;
      const currentPrice = meta.preMarketPrice || meta.postMarketPrice || meta.regularMarketPrice || meta.chartPreviousClose;
      
      return res.status(200).json({ symbol: meta.symbol || finalSymbol, name: resolvedName, currentPrice: Number(currentPrice), prevClose: Number(meta.chartPreviousClose || currentPrice), apiVersion: 'v7.8.5 (Yahoo Backup)' });
    }
  } catch (error) {
